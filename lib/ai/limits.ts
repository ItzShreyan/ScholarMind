...
type ReservationResult =
  | ({ allowed: true; token: string; persisted: boolean } & AiUsageSummary)
  | ({ allowed: false; persisted: boolean } & AiUsageSummary);
...
function getWindowResetInfo(entries: UsageEntry[], now: number, windowMs = 1000 * 60 * 60 * 24) {
  const activeEntries = pruneEntries(entries, now, windowMs);
  if (!activeEntries.length) {
    return { resetAt: now, resetInMs: 0 };
  }

  const oldestEntry = activeEntries.reduce((oldest, entry) => (entry.timestamp < oldest.timestamp ? entry : oldest));
  return {
    resetAt: oldestEntry.timestamp + windowMs,
    resetInMs: Math.max(0, oldestEntry.timestamp + windowMs - now)
  };
}
...
function reserveInMemory(
  ...
  const reset = getWindowResetInfo(currentEntries, now);
  ...
  return {
    ...
    hourlyLimit,
    dailyLimit,
    resetAt: getWindowResetInfo(currentEntries, now).resetAt,
    resetInMs: getWindowResetInfo(currentEntries, now).resetInMs,
    persisted: false
  };
}
...
async function reserveInSupabase({
  ...
  try {
    ...
    const reset = getWindowResetInfo(
      rows.map((row) => ({ id: row.id, timestamp: new Date(row.created_at).getTime() })),
      now
    );
    ...
    return {
      ...
      hourlyLimit,
      dailyLimit,
      resetAt: getWindowResetInfo(
        [...rows.map((row) => ({ id: row.id, timestamp: new Date(row.created_at).getTime() })), { id: token, timestamp: now }],
        now
      ).resetAt,
      resetInMs: getWindowResetInfo(
        [...rows.map((row) => ({ id: row.id, timestamp: new Date(row.created_at).getTime() })), { id: token, timestamp: now }],
        now
      ).resetInMs,
      persisted: true
...
export function formatAiLimitMessage(
  ...
  const message = `Study tool limit reached. You can run up to ${dailyLimit} study tool generation${dailyLimit === 1 ? "" : "s"} per day in the free preview.`;
  const resetHours = usage?.resetInMs != null ? Math.max(1, Math.ceil(usage.resetInMs / (1000 * 60 * 60))) : 0;
  return resetHours > 0 ? `${message} Resets in ~${resetHours} hour${resetHours === 1 ? "" : "s"}.` : message;
}
